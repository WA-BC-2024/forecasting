...
* `plot(mod_fit)` to view a series of plots of the states, data, and diagnostics
...
```{r}
## fit the model with first 30 data points
mod_fit <- MARSS(y = YY, model = mod_list)

fY <- predict(mod_fit, type="ytt1")

fX <- predict(mod_fit, type="xtt1")

fX$pred$estimate

kf <- MARSSkfas(mod_fit)
