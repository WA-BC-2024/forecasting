...
```{r, dpi=300, out.height = "90%", out.width = "90%", fig.align = 'center', echo = FALSE}
theta <- coef(lm(yy ~ seq(TT)))
y_obs <- theta[1] + theta[2] * seq(TT)
...
```{r ex_biased_rw, fig.dim = c(8,4), dpi=300, out.height = "90%", out.width = "90%", fig.align = 'center', echo = FALSE}
...
par(mfrow = c(1,2), mai = c(1, 1, 0.5, 0))
...
```{r brw_states_obs_0, dpi=300, out.height = "90%", out.width = "90%", fig.align = 'center', echo = FALSE}
...
# Model options | Biased random walk + obs error
...
```{r brw_states_obs, dpi=300, out.height = "90%", out.width = "90%", fig.align = 'center', echo = FALSE}
xw <- rbind(cbind(seq(TT), xx), cbind(seq(TT)+0.5, xx+uu))
xw <- xw[order(xw[,1]),]
xw[,1] <- c(1, rep(seq(2, TT), ea = 2), TT)
...
```{r brw_states_obs_2, dpi=300, out.height = "90%", out.width = "90%", fig.align = 'center', echo = FALSE}
par(mai = c(0.9,0.9,0.1,0.1), omi = c(0,0,0,0))
## blank slate
plot.ts(yy, ylim = range(xx,yy), type = "n",
        las = 1, ylab = expression(italic(x[t])))
...
# Variance in $y_t$ is due to both process & obs error
...
> # .blue[How is it possible to separate out both the process and observation errors?]
